...
## Testing Guidelines
There is no separate `tests/` directory; successful compilation is the test suite. Run `lake build` before opening a PR, and rerun `lake env lean <file>` on any module you touched. When adding a theorem, verify downstream files still compile, especially [`LowRankUnivariateSOS/Basic.lean`](./LowRankUnivariateSOS/Basic.lean), which imports the full library.
